...
int main() {
    ...
    // fr(i, n) for (auto x: div[i]) dbgpt(x);
    ...
    fr(i, n) {
        ...
        if (i%2) {
            ...
            f.add_edge(mp[{i,0}], snk, INF);

            for (auto x: div[i])
                f.add_edge( mp[{i,x.f}], mp[{i,0}], x.s );

            continue;
        }

        f.add_edge(src, mp[{i,0}], INF);

        for (auto x: div[i]) {
            f.add_edge( mp[{i,0}], mp[{i,x.f}], x.s );

            for (auto j: adj[i]) 
                if (mp.find({j, x.f}) != mp.end())
                    f.add_edge(mp[{i,x.f}], mp[{j,x.f}], INF);
        }
